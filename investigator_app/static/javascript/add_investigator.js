...
function add_skills(){
    ...
    input_name.setAttribute("onclick", "this.select();");
    ...
    input_initial.setAttribute("onclick", "this.select();");
    ...
    input_job.setAttribute("onclick", "this.select();");
    ...
    input_interest.setAttribute("onclick", "this.select();");
    ...
    input_growth.setAttribute("onclick", "this.select();");
    ...
    input_others.setAttribute("onclick", "this.select();");
...
// 技能json化
function jsonify_skill(skills){
    var skills_json = "[";
    let skills_count = skills.childElementCount - 1;
    var skill = skills.firstElementChild;
    for(let i = 0; i < skills_count; i++){
        ...
        skill_element = skill_element.nextElementSibling;
        var skill_add = skill_element.firstElementChild;

        if(skill_name.value == "" || skill_name.value == null){skill_name.value = " ";}
        if(skill_initial.value == "" || skill_initial.value == null){skill_initial.value = "0";}
        if(skill_job.value == "" || skill_job.value == null){skill_job.value = "0";}
        if(skill_interest.value == "" || skill_interest.value == null){skill_interest.value = "0";}
        if(skill_growth.value == "" || skill_growth.value == null){skill_growth.value = "0";}
        if(skill_others.value == "" || skill_others.value == null){skill_others.value = "0";}

        skills_json = skills_json + "{\"id\": " + skill_check.value +", ";
        skills_json = skills_json + "\"name\": \"" + skill_name.value + "\", ";
        if(skill_add.nodeName = "input"){
            skills_json = skills_json + "\"spell\": \"" + skill_add.value + "\", ";
        }
        skills_json = skills_json + "\"points\": {"
        skills_json = skills_json + "\"initial_points\": " + skill_initial.value + ", ";
        skills_json = skills_json + "\"job_points\": " + skill_job.value + ", ";
        skills_json = skills_json + "\"interest_points\": " + skill_interest.value + ", ";
        skills_json = skills_json + "\"growth_points\": " + skill_growth.value + ", ";
        skills_json = skills_json + "\"others_points\": " + skill_others.value;
        skills_json = skills_json + "}}"
    }
    skills_json = skills_json + "]";
    return skills_json;
...
// 通過セッションリスト更新
function change_session_list(){
    var sessions_json = '[';
    var sessions_element = document.getElementById('sessions').firstElementChild;
    let sessions_count = sessions_element.childElementCount-1;
    sessions_element = sessions_element.firstElementChild;
    for(let i = 0; i < sessions_count; i++){
        sessions_element = sessions_element.nextElementSibling;
        session = sessions_element.firstElementChild.firstElementChild.value;
        if(session) {
            if(sessions_json != '['){
                sessions_json = sessions_json + ', '
            }
            sessions_json = sessions_json + '"' + session + '"';
        }
        
    }
    sessions_json = sessions_json + ']';
    
    const session_list = document.getElementById('session_list');
    session_list.value = sessions_json
...
function add_session(){
    ...
    input.setAttribute("onchange", "change_session_list()")

    let button = document.createElement("button")
    button.type = "button"
    button.textContent = "削除"
    button.style.marginLeft = "3px"
    button.setAttribute("onclick", "delete_session(this)")
    ...
    sessions.firstElementChild.appendChild(tr)
}
...
window.addEventListener('DOMContentLoaded', function(){
    setInterval(() => {
        ...
        // 回避
        var dodge_initial_points = document.getElementById('dodge_initial_points');
        dodge_initial_points.value = Number(dex_sum.value) * 2;
        
        // 母国語
        var own_language_initial_points = document.getElementById('own_language_initial_points');
        own_language_initial_points.value = Number(edu_sum.value) * 5;
        ...
        // 技能処理
            ...
            battle_skills_json.value = jsonify_skill(battle_skills);
            // 探索技能処理
            ...
            search_skills_json.value = jsonify_skill(search_skills);
            // 行動技能処理
            ...
            action_skills_json.value = jsonify_skill(action_skills);
            // 交渉技能処理
            ...
            interpersonal_skills_json.value = jsonify_skill(interpersonal_skills);
            // 知識技能処理
            ...
            knowledge_skills_json.value = jsonify_skill(knowledge_skills);

        // SAN値
        let cm_sum_points = Number(document.getElementById('cthulhu_mythos_sum_points').value)
